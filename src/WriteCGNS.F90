...
!h160123 Xrain�Ή�
		...
		double precision, ALLOCATABLE, DIMENSION(:,:) :: Rdata	!h160123 Xrain�Ή�
        integer, ALLOCATABLE, DIMENSION(:,:) :: ibc0	!h160123 Xrain�Ή�
		...
		do i = 1, j_in
			CALL CG_IRIC_WRITE_SOL_BASEITERATIVE_REAL(fid, trim(fname_in(i)), disch1(i), IER)
        end do
        
        if(j_side_j1.eq.2) then
		do i = 1, jsin1
		    CALL CG_IRIC_WRITE_SOL_BASEITERATIVE_REAL(fid,trim(fname_1(i)), disch2(i), IER)
		end do
        end if
		...
		do i = 1, jsin2
!docon edit start#####################################################################################
!			CALL CG_IRIC_WRITE_SOL_BASEITERATIVE_REAL(fid,trim(fname_2(i)), disch3(1), IER)
			CALL CG_IRIC_WRITE_SOL_BASEITERATIVE_REAL(fid,trim(fname_2(i)), disch3(i), IER)   !140602
!docon edit end#######################################################################################
		end do
		end if
...
!docon add start#####################################################################################
		...
		do i = 1, bc_num
			CALL CG_IRIC_WRITE_SOL_BASEITERATIVE_REAL(fid,trim(Bcname(i)), bc_qswap(i), IER)
		end do
		end if

        if(gt_num.ge.1) then
		do i = 1, gt_num
			CALL CG_IRIC_WRITE_SOL_BASEITERATIVE_REAL(fid,trim(Gtname(i)), gt_qswap(i), IER)
		end do
		end if

        if(p_num.ge.1) then
		do i = 1, p_num
			CALL CG_IRIC_WRITE_SOL_BASEITERATIVE_REAL(fid,trim(Ppname(i)), p_qswap(i), IER)
		end do
		end if
!docon add end#######################################################################################
		...
		ALLOCATE(Rdata(nx, ny), STAT=ier)	!h160123 Xrain�Ή�
