...
!docon add end#######################################################################################
		...
		call cg_iric_read_bc_count(fid, 'inflow', j_in)
      ...
      allocate(flowname(j_in))
		...
		do i = 1, j_in
			call cg_iric_read_bc_indicessize(fid,'inflow', i, j_inlen(i), ier)

			if(indexmax.lt. j_inlen(i)) indexmax= j_inlen(i)

            call cg_iric_read_bc_string(fid,'inflow',i,'_caption',flowname(i), ier)
			call cg_iric_read_bc_functionalsize(fid,'inflow', i, 'qt1', j_size(i), ier);
		...
		do i = 1, j_in
			call cg_iric_read_bc_indices(fid,'inflow', i, indices(i:i,:,:), ier)
			call cg_iric_read_bc_real(fid,'inflow', i, 'slope1', slopevalue(i:i), ier)
			call cg_iric_read_bc_functional(fid,'inflow',i, 'qt1', f_param(i:i,:), f_value(i:i,:), ier)
		end do

!docon add start#####################################################################################
        !以下、ボックスカルバート・樋門・ポンプの境界条件追加
		call cg_iric_read_bc_count(fid,'BoxCulvert', bc_num)
		call cg_iric_read_bc_count(fid,'Gate', gt_num)
		call cg_iric_read_bc_count(fid,'Pump', p_num)
        ...
        do i=1,bc_num   !データ数分繰り返し
			call cg_iric_read_bc_indicessize(fid,'BoxCulvert', i, bc_mxindx(i), ier)  !セル数取得
            call cg_iric_read_bc_string(fid,'BoxCulvert',i,'_caption',BCname(i), ier) !140602
            ...
            call cg_iric_read_bc_INTEGER(fid,'BoxCulvert', i, 'bc_couple_num', bc_couple_num(i), ier)
            call cg_iric_read_bc_INTEGER(fid,'BoxCulvert', i, 'bc_inout', bc_inout(i), ier)   !入口=0,出口=1
            if(bc_inout(i)==0) then         !入口側の場合
                call cg_iric_read_bc_INTEGER(fid,'BoxCulvert', i, 'bc_c2_yesno', bc_c2_yesno(i), ier)
                call cg_iric_read_bc_REAL(fid,'BoxCulvert', i, 'bc_width', bc_width(i), ier)
                call cg_iric_read_bc_REAL(fid,'BoxCulvert', i, 'bc_height', bc_height(i), ier)
                call cg_iric_read_bc_REAL(fid,'BoxCulvert', i, 'bc_base', bc_base(i), ier)
                call cg_iric_read_bc_REAL(fid,'BoxCulvert', i, 'bc_ren', bc_ren(i), ier)
                call cg_iric_read_bc_REAL(fid,'BoxCulvert', i, 'bc_c1', bc_c1(i), ier)
                call cg_iric_read_bc_REAL(fid,'BoxCulvert', i, 'bc_c3', bc_c3(i), ier)
                if(bc_c2_yesno(i)==1) then  !中間流出考慮の場合
                    call cg_iric_read_bc_REAL(fid,'BoxCulvert', i, 'bc_c2', bc_c2(i), ier)
                end if
            end if
        end do
        ...
        do i=1,bc_num   !データ数分繰り返し
			call cg_iric_read_bc_indices(fid,'BoxCulvert', i, bc_indx(i,:,:), ier)
        ...
        do i=1,gt_num   !データ数分繰り返し
			call cg_iric_read_bc_indicessize(fid,'Gate', i, gt_mxindx(i), ier)    !セル数取得
            call cg_iric_read_bc_string(fid,'Gate',i,'_caption',Gtname(i), ier)   !140602
	        ...
	        call cg_iric_read_bc_INTEGER(fid,'Gate', i, 'gt_couple_num', gt_couple_num(i), ier)
	        call cg_iric_read_bc_INTEGER(fid,'Gate', i, 'gt_inout', gt_inout(i), ier)
            if(gt_inout(i)==0) then         !入口側の場合
    	        call cg_iric_read_bc_INTEGER(fid,'Gate', i, 'gt_method', gt_method(i), ier)
	            call cg_iric_read_bc_INTEGER(fid,'Gate', i, 'gt_c2_yesno', gt_c2_yesno(i), ier)
	            call cg_iric_read_bc_REAL(fid,'Gate', i, 'gt_width', gt_width(i), ier)
	            call cg_iric_read_bc_REAL(fid,'Gate', i, 'gt_height', gt_height(i), ier)
	            call cg_iric_read_bc_REAL(fid,'Gate', i, 'gt_base', gt_base(i), ier)
	            call cg_iric_read_bc_REAL(fid,'Gate', i, 'gt_ren', gt_ren(i), ier)
	            call cg_iric_read_bc_REAL(fid,'Gate', i, 'gt_c1', gt_c1(i), ier)
	            call cg_iric_read_bc_REAL(fid,'Gate', i, 'gt_c3', gt_c3(i), ier)
                if(gt_c2_yesno(i)==1) then  !中間流出考慮の場合
                    call cg_iric_read_bc_REAL(fid,'Gate', i, 'gt_c2', gt_c2(i), ier)
                end if
		        if(gt_method(i)==1) then
		            call cg_iric_read_bc_functionalsize(fid,'Gate', i, 'gt_openclose', gt_size(i), ier);
		            if (gt_tmax < gt_size(i)) gt_tmax = gt_size(i)
		        end if
		    end if
	        if(gt_inout(i)==1) then         !出口側の場合
	            call cg_iric_read_bc_INTEGER(fid,'Gate', i, 'gt_outwl', gt_outwl(i), ier)
	            if(gt_outwl(i)==1) then     !外水位が外読みファイルで与えられているとき
	                call cg_iric_read_bc_functionalsize(fid,'Gate', i, 'wl1', gt_size2(i), ier)
        ...
        do i=1,gt_num   !データ数分繰り返し
			call cg_iric_read_bc_indices(fid,'Gate', i, gt_indx(i,:,:), ier)
			if(i==1) write(*,*) '------------------------Gate------------------------'
            ...
            if(gt_inout(i)==0) then     !入口側の場合
    			...
    			call cg_iric_read_bc_functional(fid,'Gate', i, 'gt_openclose', gt_time(i,:), gt_state(i,:), ier)
            end if
	        if(gt_inout(i)==1) then     !出口側の場合
	            call cg_iric_read_bc_INTEGER(fid,'Gate', i, 'gt_outwl', gt_outwl(i), ier)
	            if(gt_outwl(i)==1) &    !外水位fid,が外読みファイルで与えられているとき
	            call cg_iric_read_bc_functional(fid,'Gate', i, 'wl1', gt_time2(i,:), gt_wl(i,:), ier)
            end if
        end do
        ...
        do i=1,p_num    !データ数分繰り返し
			call cg_iric_read_bc_indicessize(fid,'Pump', i, p_mxindx(i), ier) !セル数取得
            call cg_iric_read_bc_string(fid,'Pump',i,'_caption',Ppname(i), ier)   !140602
            !複数メッシュの選択は不可
       ...
       !     end if
	        ...
	        call cg_iric_read_bc_INTEGER(fid,'Pump', i, 'p_couple_num', p_couple_num(i), ier)
	        call cg_iric_read_bc_INTEGER(fid,'Pump', i, 'p_inout', p_inout(i), ier)
            if(p_inout(i)==0) then          !入口側の場合
	            call cg_iric_read_bc_INTEGER(fid,'Pump', i, 'p_method', p_method(i), ier)
	            if(p_method(i)==1) then     !ポンプ排水量が外読みファイルで与えられているとき
		            call cg_iric_read_bc_functionalsize(fid,'Pump', i, 'qt2', p_size(i), ier);
                end if
	            call cg_iric_read_bc_REAL(fid,'Pump', i, 'p_qmax', p_qmax(i), ier)
	            call cg_iric_read_bc_REAL(fid,'Pump', i, 'p_startope', p_startope(i), ier)
	            call cg_iric_read_bc_REAL(fid,'Pump', i, 'p_stopope', p_stopope(i), ier)
            end if
        ...
        do i=1,p_num   !データ数分繰り返し
			call cg_iric_read_bc_indices(fid,'Pump', i, p_indx(i,:,:), ier)
			if(i==1) write(*,*) '------------------------Pump------------------------'
            ...
            if(p_inout(i)==0) then          !入口側の場合
	            if(p_method(i)==1) then     !ポンプ排水量が外読みファイルで与えられているとき
	                call cg_iric_read_bc_functional(fid,'Pump', i, 'qt2', p_time(i,:), p_qout(i,:), ier)
                end if
            end if
            if(p_inout(i)==1) then          !出口側の場合
                call cg_iric_read_bc_functional(fid,'Pump', i, 'qt2', p_time(i,:), p_qout(i,:), ier)
